...
int connectToSON() { 
  ...
  int son_conn = socket(AF_INET, SOCK_STREAM, 0);
  memset(&sonaddr, 0, sizeof(sonaddr));
  sonaddr.sin_family = AF_INET;
  sonaddr.sin_addr.s_addr = inet_addr("127.0.0.1");
  sonaddr.sin_port = htons(SON_PORT);
  printf("connect to son...\n");
  if(!connect(son_conn, (struct sockaddr*)&sonaddr, sizeof(sonaddr))) {
	printf("connected\n");
	return son_conn;
  }
  else 
...
void* routeupdate_daemon(void* arg) {
  ...
  while (1) {
	printf("send out a route update packet\n");
	son_sendpkt(BROADCAST_NODEID, &pkt, gson_conn);
...
void* pkthandler(void* arg) {
	...
	while(son_recvpkt(&pkt,gson_conn)>0) {
...
//它关闭所有连接, 释放所有动态分配的内存.
void sip_stop() {
  fprintf(stderr, "in sip stop\n");
  while (close(gson_conn)!=0) {}
  exit(0);
}
